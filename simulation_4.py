...
def ode(t, y, config):
    ...
    output[0] = y[1]
    if t < config["t_burn"]:
        if y[2] > config["dry_mass"] + 0.1 * config["fuel_mass"]:
        ...
        if y[2] > config["dry_mass"]:
            output[1] = (1 / y[2]) * (-config["fuel_consumption"] * config["fuel_density"] * config["throttle_rate"]) * (config["isp"] * -g) + \
                -g + drag / y[2]
            output[2] = -config["fuel_consumption"] * config["fuel_density"] * config["throttle_rate"]
        else:
            output[1] = -g + drag / y[2]
            output[2] = 0
...
def get_acceleration(y, config):
    t_burn = int(np.ceil(config["t_burn"] * 2))
    g = config["g"] * ((config["r_earth"] / (config["r_earth"] + y[0, :])) ** 2)
    g_accleeration = -g
    ...
    burn_acceleration[y[2, :] <= config["dry_mass"] + 0.1 * config["fuel_mass"]] = 0
    burn_acceleration[t_burn:] = 1 / y[2, t_burn:] * (-config["fuel_consumption"] * config["fuel_density"] * config["throttle_rate"]) * (config["isp"] * -g[t_burn:])
...
if __name__ == "__main__":
    with open("config_4.yaml", 'r') as f:
